...
use TotalFormVendors\Psr\Http\Message\UriInterface;
...
class Uri implements UriInterface
{
    ...
    public function __construct(
        ...
        $this->path     = ($path === null || $path === '') ? '/' : $this->filterPath($path);
     ...
     * @return UriInterface
     */
    public static function createFromString($uri): UriInterface
    {
     ...
     * @return UriInterface
     */
    public static function createFromServer(ServerContext $context): UriInterface
    {
        // Scheme
        $isSecure = $context->get('HTTPS');
        $scheme   = (empty($isSecure) || $isSecure === 'off') ? 'http' : 'https';

        // Authority: Username and password
        $username = $context->get('PHP_AUTH_USER', '');
        $password = $context->get('PHP_AUTH_PW', '');
        ...
        if ($context->has('HTTP_HOST')) {
            ...
            // set a port default
            $port = (int)$context->get('SERVER_PORT', 80);
        }
        ...
        // Path
        $requestScriptName = (string)parse_url($context->get('SCRIPT_NAME'), PHP_URL_PATH);
        $requestScriptDir  = dirname($requestScriptName);

        // parse_url() requires a full URL. As we don't extract the domain name or scheme,
        // we use a stand-in.
        $requestUri = (string)parse_url('http://example.com' . $context->get('REQUEST_URI'), PHP_URL_PATH);
        ...
        // Query string
        $queryString = $context->get('QUERY_STRING', '');
        if ($queryString === '') {
            $queryString = parse_url('http://example.com' . $context->get('REQUEST_URI'), PHP_URL_QUERY);
        }
